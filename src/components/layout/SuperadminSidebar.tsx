...
export function SuperadminSidebar({ mobileOpen, onMobileClose }: SuperadminSidebarProps) {
  const { collapsed, expand, collapse } = useSuperadminSidebarStore();
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-40 flex flex-col bg-[var(--sidebar)] border-r border-blue-500/30 transition-all duration-300',
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between h-16 px-4 border-b border-blue-500/30 shrink-0">
          {(!collapsed || mobileOpen) && (
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-blue-500 text-[10px] font-medium">Admin Panel</p>
              </div>
            </div>
          )}
          {collapsed && !mobileOpen && (
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    ...
                    'text-[var(--sidebar-foreground)] hover:bg-blue-500/10 hover:text-blue-600 dark:hover:text-blue-400',
                    isActive && 'bg-blue-500/15 text-blue-600 dark:text-blue-400 font-semibold',
          ...
          <div className="mt-1">
            <button
              ...
              className={cn(
                ...
                'text-[var(--sidebar-foreground)] hover:bg-blue-500/10 hover:text-blue-600 dark:hover:text-blue-400',
                logsOpen && 'bg-blue-500/10 text-blue-600 dark:text-blue-400',
                collapsed && !mobileOpen && 'md:justify-center md:px-0'
              )}
            ...
            {logsOpen && (!collapsed || mobileOpen) && (
              <div className="ml-4 pl-3 border-l border-blue-500/20 mt-0.5 space-y-0.5">
        ...
        {/* User + logout */}
        <div className="shrink-0 px-2 pb-3 border-t border-blue-500/20 pt-3 space-y-1">
          {(!collapsed || mobileOpen) && user && (
            <div className="px-2.5 py-2 rounded-md bg-blue-500/10 mb-2">
              <p className="text-xs font-medium text-[var(--sidebar-foreground)] truncate">{user.name}</p>
              <p className="text-[10px] text-blue-500 font-medium">Superadmin</p>
            </div>
          )}
          <button
